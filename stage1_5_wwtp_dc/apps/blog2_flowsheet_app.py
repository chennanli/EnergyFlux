...
# ===========================================================================
with st.sidebar:
    # ── Action buttons (moved here from top bar so they don't get clipped) ─
    st.markdown("### ⚙️ Actions")
    sb_act = st.columns(3)
    if sb_act[0].button("📖 Defaults", use_container_width=True,
    ...
    if sb_act[1].button("🔄 Reset", use_container_width=True,
                        help="Reset to default Austin 23-ac design"):
        st.session_state.design = recompute_all(default_design())
        st.session_state.selected_block = "pv"
        st.rerun()
    if sb_act[2].button("📥 Export", use_container_width=True,
                        help="Show the current design as JSON"):
        import json
    ...
    model = st.session_state.get("copilot_model", "meta/llama-3.1-70b-instruct")
    render_copilot_panel(
